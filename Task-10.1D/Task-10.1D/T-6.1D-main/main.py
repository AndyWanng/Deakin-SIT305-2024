...
def fetchSummaryFromChatGPT(text):
    headers = {
        'Authorization': 'Bearer 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',
        'Content-Type': 'application/json'
    }
    data = {
        'model': 'gpt-3.5-turbo',
    ...
    if response.status_code == 200:
        ...
        try:
            ...
            app.logger.debug(f"Extracted text content: {text_content}")
            return text_content
        except (IndexError, KeyError) as e:
            app.logger.error(f"Error parsing response data: {e}")
            return None
    else:
        app.logger.error(f"Error in API call: {response.status_code} {response.text}")
        return None
...
def get_summary():
    ...
    response_text = fetchSummaryFromChatGPT(content)
    if response_text:
        return jsonify({
            'choices': [
        ...
        return jsonify({'error': 'Failed to fetch summary from OpenAI'}), 500
